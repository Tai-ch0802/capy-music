...
ids=($(curl -sS "${H[@]}" \
  "${BASE}/catalog/tw/search?types=songs&limit=2&term=$(jq -rn --arg s '五月天' '$s|@uri')" \
...
curl -sS -o /dev/null -w '  add tracks → HTTP %{http_code}\n' -X POST "${H[@]}" \
  ...
  "${BASE}/me/library/playlists/${PL}/tracks"

echo "④ 讀回清單曲目(取得 library track id)"
tracks="$(curl -sS "${H[@]}" "${BASE}/me/library/playlists/${PL}/tracks")"
...
curl -sS -o /dev/null -w '  DELETE …/tracks/{id}       → HTTP %{http_code}\n' \
  -X DELETE "${H[@]}" "${BASE}/me/library/playlists/${PL}/tracks/${LT0}"
