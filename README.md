...
- **A few commands aren't offered on the web**: the `debug` group, `--auto`, `--client-secret` / `--developer-token` / `--user-token` (use the wizard instead) and `now --watch` (just watch the panel). `capy update` can run, but after updating, this web process is still the old version and asks you to restart it.
