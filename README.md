...
The other pages: **My playlists**, **Sync**, **Search** and **Accounts**; under "Advanced" are the **Console** (run any subcommand, just like in a terminal), **ISRC lookup** (asks all three platforms at once) and **Diagnostics**. The bottom of every page always has the **now-playing bar** (the playback panel): what's playing, progress and playback controls — everything `capy now` shows. It follows whichever platform is actually playing (only a platform that is playing takes it over, a paused one doesn't), its controls act on the platform it shows, and `capy --web --provider X` pins it to one platform. Every action on the page is a capy command underneath, and the Console keeps the full log; anything that needs confirming is always asked by the command itself, and the page never answers for you. The sidebar also has the language menu (see [Language](#language)).
