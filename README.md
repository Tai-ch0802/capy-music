...
按 `/` 開命令選單:打字即時過濾,`↑↓` 選,`⏎` 把命令**帶進輸入行**(不直接執行,你還可以補參數),
`Esc` 收起。選單向上長、壓在捲動區前面,不插進歷程。清單直接來自 cobra 的命令樹,不會跟實作走鏢。
...
- **`--create` 建的清單跟 canonical 同名**,所以 push 不會多一列 `rename`。在終端機裡也可以直接打 `capy pl link`,第二段選「在 spotify 建一個新的空清單」。
