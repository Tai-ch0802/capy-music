...
完整的使用說明(含互動式介面、命令參考與三個平台的能力差異)在 `docs/guide.html`。GitHub 對 repo 裡的 `.html` 只顯示原始碼,所以要看那一頁請 clone 或下載後用瀏覽器打開——它是單一檔案,不需要伺服器,也不連任何外部資源。
