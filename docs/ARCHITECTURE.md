...
capy pl dedup  [name|pid | <provider>:<playlist_id|name>] [--provider P] [--dry-run] [--yes] [--force]   # 2026-09-15(決策 38;internal/cli/dedup.go):重複 = 同平台 id 或同 ISRC(cid 的定義;單曲版 / 專輯版算同一首),保留第一份、其餘順序與 rank 不動。<provider>:<清單> 直接讀平台、只報告(TSV:pos id title artists reason;不碰 Drive、不需連結;Apple 只讀只能到這裡;--yes / --force / --provider 配它是錯誤);canonical 清單 = sync 的一輪中間多一步(pull 半邊 → C 去重 → push 半邊拿掉可寫平台上多的份),dir 多一種 dedup(pos 是 C 的位置)、閾值去重與 push 各算、正本與平台都沒重複時零寫入;寫不了的平台上剩下的份列 stderr 請手動刪,規則 4′ 不會把它們加回 C
