...
| Library playlist 寫入 | **2026-09-22 真帳號驗過全部可行**(計畫 2026-09-22-apple-write.md §5):建 `POST /me/library/playlists` 201、append `POST …/tracks` 204(官方端點);重排 / 移除用 `PUT …/tracks` 整批取代 204(列 id + `library-songs`,可混新曲 catalog id + `songs`;重複曲目依出現次數精準)、改名 `PATCH` 204(只送 name,描述保留)、刪清單 `DELETE` 204(amp-api 私有端點,Apple 未承諾)。只有 `canEdit:true`(= 使用者自建)的清單可寫,其餘回 500。新清單 2 s 可按 id 讀到、9 s 進列表;加進清單的曲目同時進資料庫(列 id 是 `i.`) | 決策 49(取代 30 的 append-only 預期);不用 `DELETE …/tracks`(`mode=all` 會把同一首兩列一起刪、不帶 ids 清空整份);`CreatePlaylist` 要等列表出現(pull 的 gone 判準);429 無 Retry-After、滾動約一小時 |
