...
| macOS 本機播放(Music.app) | AppleScript `play` 只收資料庫裡的曲目或檔案,曲目物件上沒有 catalog id:純 AppleScript 播不了不在資料庫的目錄歌曲,深層連結也都不會播。2026-09-24 macOS 26.5.1 實測(計畫 2026-09-24-web-player-follow-apple-play.md §2.2):`open location` 連頁面都不換;專輯網址 `?i=` 只到專輯頁;shell `open "music://music.apple.com/<storefront>/song/<id>"` 跳到專輯頁並標亮那一首,不需要自動化權限;標亮後下 `play` 播的是原本暫停的那首。2026-09-27:AppleScript 播資料庫裡的訂閱雲端曲目(shared track)可行,約 0.8 秒後 `current track` 的 persistent ID 就是那一首 | 決策 52:資料庫裡剛好對到一首才用 AppleScript `play` 並確認真的在播;其他一律開單曲網址、照實說沒播(不印 ▶、exit 0);Windows 照舊不支援播放 |
| Library playlist 寫入 | **2026-09-22 真帳號驗過**(計畫 2026-09-22-apple-write.md §5;拋棄式清單,列 id 全是 `i.`):建 `POST /me/library/playlists` 201、append `POST …/tracks` 204(官方端點);`PUT …/tracks` 整批取代 204(`i.` 列 + `library-songs`,可混新曲 catalog id + `songs`;重複曲目依出現次數精準)、改名 `PATCH` 204(只送 name,描述保留)、刪清單 `DELETE` 204(amp-api 私有端點,Apple 未承諾);新清單 2 s 可按 id 讀到、9 s 進列表。加進清單的曲目**不一定**進資料庫(2026-09-23 更正:列 id 的 `i.` 只是資料庫命名空間;這個帳號自己的一般清單就有大量不在資料庫的歌,探測加進清單的歌也沒留在資料庫,看帳號的 Apple Music 設定)。**社群佐證、未自驗**:`canEdit:false` 的清單寫入回 500「Unable to update tracks」(epheterson;R-8 沒對任何 `canEdit:false` 清單發過寫入,也不該發);**補測(同日,使用者授權)**:`a.` 列只出現在**協作清單**(`hasCollaboration:true`;其他 6 個自建清單 25–543 列全是 `i.`),對它原序全量 PUT 222 列 → **500「Unable to update tracks」、零變動**(當時分不出是 `a.` id、協作本身、還是**列數**;列數已由補測 3 排除)→ 清單列表帶 `PlaylistRef.Unwritable`(canEdit:false、協作),push 在 plan 階段跳過那一格(sync / cron 不紅、dry-run 看得到);`ApplyOps` 第二道防線;非協作 `i.` 列、Music.app 建的「跳跳兒歌」(25 列)原序全量 PUT 204 零變動(2026-09-23 補測 2);**列數排除到 538 列**:用「太好聽」的歌建私人拋棄式複本、一次反序 PUT 538 列 204 且真的倒過來(補測 3,原清單不寫);POST 每批 100 驗過;商店裡已下架的 catalog id(tw 商店 GET 回 404),POST 回 204 但默默不加(capy 寫完重讀會警告);協作清單維持唯讀(使用者 2026-09-23 定案) | 決策 49(取代 30 的 append-only 預期);不用 `DELETE …/tracks`(`mode=all` 會把同一首兩列一起刪、不帶 ids 清空整份);`CreatePlaylist` 要等列表出現(pull 的 gone 判準);429 無 Retry-After、滾動約一小時 |
...
**TUI 與可腳本化【定案】:** TUI 第一天進場(charmbracelet 全家桶),但**所有命令在非 TTY(pipe/cron)下必須可純文字輸出**——「可納入 cron/script」是 §8.5.6 列的核心價值,不得被 TUI 犧牲。互動場景:無參數執行 `capy` 的儀表板、`now --watch`、`resolve --review`、auth 精靈(huh 表單,含 BYO Client ID onboarding)。**第三種模式(2026-09-17,P7 決策 40)**:`capy --web` 在同一個行程裡執行命令、輸出非 TTY(writer 不是 `*os.File`,TSV / 表格結構化送到瀏覽器)、互動提示由同一組接縫換成瀏覽器回答——TUI 用 re-exec 是因為終端機有 TTY 可讓給子行程,瀏覽器沒有;非 TTY 純文字契約不因 web 改變。**多語系之後(2026-09-23,決策 50)這份契約的範圍是機器欄位**:TSV 欄名與既有欄位的位置、`action` / `dir` 的值、`REASON_CODE`、exit code 不變,新欄只加在最後;給人看的文字(`REASON`、stderr、說明、提示)跟著 config 的 `language`。
...
**exit code**:`0` 無事可寫或已寫入——**review 佇列有東西仍是 0**(它是 TSV 列,不是失敗;cron 的 `resolve --yes` 不能因為永遠有幾首解不開而永遠報錯)、`1` 錯誤、`2` 有待寫入的自動 mapping 但沒有確認(`--dry-run`、非 TTY 沒給 `--yes`、終端機取消)。非 TTY 的 TSV 欄位:`action cid provider provider_id confidence source title artists reason reason_code`,`action ∈ map | review | conflict`(`reason_code` 是給腳本的固定代碼、永不翻譯;`reason` 給人看、跟著語系——決策 50;pull / push / sync / migrate / dedup 的 TSV 同樣以這兩欄結尾)。
...
9. **輸出**:變更集(add / remove / move / rename / unlink,欄位對齊 T8 的 TSV:`action pos cid provider_id title artists reason reason_code`)、套用後的 C、新建或更新的 tracks、新的 base 快照(= 平台清單 id、L 的 provider id 原文與觀測當時的 cid;§6.3)、變更前該 provider **可見**的 item 數(cid 有該 provider mapping 的數量,Q3 的閾值分母)。
...
| 網頁介面的邊界 | `capy --web`(2026-09-17,CLAUDE.md 硬約束、附錄 C 決策 40–41):只綁 127.0.0.1、不可用 8888、80、443(80 / 443:瀏覽器會把預設埠號從 `Host` / `Origin` 拿掉,逐字比對必然不符)、每次啟動一次性 token(URL fragment → `X-Capy-Token` 標頭,不用 cookie)、`Host` 逐字比對、`Origin` / `Sec-Fetch-Site` 檢查、CSP 零 inline;命令由單一序列槽在行程內執行(`/api/now`、`/api/isrc`、`/api/i18n` 三個直達端點不經 cobra、不進序列槽、不呼叫 `defaultProvider()`、不 `i18n.Set`,一樣要一次性 token 與 `Host` / `Origin` 檢查;`/api/now` 與 `/api/isrc` 仍經 `BackoffStderr` / `LockStderr` 寫提示,`/api/i18n`(2026-09-23,決策 50)只讀 config 的 `language` 與嵌入的語系目錄、不寫任何 stderr),`debug` 群組、`--auto`、`--web`、三個 secret flag 一律 403,keychain 內容與精靈輸入的 secret 絕不進事件 / log / URL;提示等待逾時 5 分鐘砍整個 job(零寫入、pull.lock 立刻放),關分頁即取消;刪除閾值、`--force` 語意、揭露不可跳過都與 CLI 相同 |
...
| **P0-2** ✅ | ~~⚠️ **驗證 Apple Music API 能否從 library playlist 移除/重排曲目**~~ 2026-09-22 真帳號驗過:`PUT …/tracks` 整批取代可移除、重排、混入新曲;`PATCH` 改名;全部 204(計畫 2026-09-22-apple-write.md §5,§1.2 表) | 不需要 rebuild;決策 49 |
...
> **排程註記(2026-09-03):** v0.4 版 P2(`.p8` + Worker + MusicKit 橋接)於 PR #5 完成後,依附錄 C 決策 8 改為 web token BYO(`feat/apple-webtoken`)。真實驗收**不再 gate 於會籍**:拿到 web token 即可跑 search / pl / 播放驗收;P0-3(MusicKit 動態 port)隨橋接移除而作廢;P0-1(ISRC)仍待真跑;macOS 播放機制 A/B 於 2026-09-24 決勝:兩者都不會播,A 連頁面都不換,改為資料庫比對後播放、比對不到就開單曲網址(決策 52)。
...
**狀態(2026-09-18)**:程式已進 main —— #66 計畫與視覺規格 v2、#67 視覺系統與外殼、#68 搬家精靈與示意動畫、#69 真實進度;T4 文件是 #70。**做下來確認的事**:精靈不需要任何新的伺服器端點——`auth status`、`pl list`、`migrate` 三條既有命令加上提示橋就夠了,新增的只有 `Console.run` 的四個選項(`label` / `args` / `promptHost` / `onPrompt`)與一個預設 no-op 的 `reportProgress` 接縫;確認一律留在 CLI,頁面不代加 `--yes`、不自動回答。**代價與已知邊角**:第一步「連接帳號」做不到一鍵(BYO 憑證,決策 8);一次搬一個清單(Q39);`write` 階段沒有逐批進度(要動 SPI);頁面靠 migrate 的兩個中文字面與兩張表的欄名認東西,由 Go 測試兩邊一起釘(2026-09-23 補註,決策 50:這些依賴都拿掉了——逐筆裁決的提示改認提示事件帶的 `key`(`migrate.confirm.review`)、「搬不搬得過去」改看 `REASON_CODE === 'push'`、欄名改成兩種語言都不翻的英文大寫、帳號狀態改讀 `auth status --json`)。**仍在維護者手上**:R-13…R-18 真帳號驗收(計畫 §2;R-14 是真的寫入),跑完才在標題打 ✅;Q40 水豚用 SVG 還是 ASCII。
...
capy pl dedup  [name|pid | <provider>:<playlist_id|name>] [--provider P] [--dry-run] [--yes] [--force]   # 2026-09-15(決策 38;internal/cli/dedup.go):重複 = 同平台 id 或同 ISRC(cid 的定義;單曲版 / 專輯版算同一首),保留第一份、其餘順序與 rank 不動。<provider>:<清單> 直接讀平台、只報告(TSV:pos id title artists reason reason_code;不碰 Drive、不需連結;Apple 只讀只能到這裡;--yes / --force / --provider 配它是錯誤);canonical 清單 = sync 的一輪中間多一步(pull 半邊 → C 去重 → push 半邊拿掉可寫平台上多的份),dir 多一種 dedup(pos 是 C 的位置)、閾值去重與 push 各算、正本與平台都沒重複時零寫入;寫不了的平台上剩下的份列 stderr 請手動刪,規則 4′ 不會把它們加回 C
capy migrate   [來源清單] --from <P> --to <P>[:<清單>] [--dry-run] [--yes]   # 2026-09-15(決策 39;internal/cli/migrate.go):一個 withCanonical 裡讀來源(不連結、不記 base)→ 決定正本(目標既有且已連 → 沿用它連的;既有沒連 → 同名(目標的名字,免得 push 排 rename)接管或建;新建 → 以來源名字找同名沿用或建,連著目標平台別的清單的擋下)→ 既有目標先 observeAndDerive(只 pull 它;閾值擋下 exit 3 指路 sync)→ 來源裡正本沒有的依來源順序 Append(同 cid 略過)→ planResolve / applyMappings(新建的給副本佔位 link;TTY 可 reviewLoop)→ 表(dir pull / migrate / push)、確認 → 確認後才 CreatePlaylist、再 observeAndDerive 記 base → planPush strict 且只准 add(移除 / 換序 / 改名 → exit 3 先 sync)→ applyPlans → finishPush(新建後 COMMIT 失敗的訊息帶 pl link 接回)。不帶參數 + TTY 四段挑選;非 TTY 要 --from / --to / --yes;Apple 不能當目標、local 只能加進既有檔
capy pl restore <name> --provider P                       # 延後(決策 31,計畫 Q18):= 把 base 快照 push 回平台,與 pl push 重疊
# pl show / link / unlink / pull / push / sync / dedup 不帶清單參數且 stdin 與 stdout 都是終端機時開挑選器;
...
capy config get|set|list       # default_provider;P6 加 local_root;2026-09-23 加 language(決策 50:en | zh-TW,沒設 = 英文;大小寫與底線寬鬆、存正規化的代碼;不支援的值被拒並列出可用的;config list 非 TTY 多一行 language,加在最後)
...
| 45 | web 面向一般使用者(2026-09-18;計畫 §1) | 預設路由改成搬家精靈(`#/move`);導覽兩層:搬家 / 我的清單 / 同步 / 搜尋 / 帳號,主控台 / ISRC 查詢 / 診斷收進「進階」,八頁、功能不減;每頁第一眼不出現命令字串(標題旁的 CLI 等價命令拿掉、按鈕說人話),命令原文仍在主控台與執行狀態列的連結;命令列只在主控台頁;主視覺保留深色 + 唯一主色 GeekGreen + 水豚,退役 mono 內文、全直角、掃描線、霓虹預算與 GitHub 灰藍底(規格 v2 §2 逐條);安全與無障礙條文一條不放寬 | 使用者 2026-09-18:「現在介面有點太工程師了 … 更面向大眾使用者 … 參考 tunemymusic」。v1 的「geek 綠」也是使用者的話,所以綠與水豚留下當品牌,退役的是「看起來像終端機」的部分;參考站本身是深色,accent 在白底不過 AA(決策 43),所以不做淺色(Q38) |
| 46 | 搬家精靈的形狀(2026-09-18;計畫 §1) | 三步(選路線 → 選清單 → 確認並搬家);只跑一次 `migrate <來源清單 ID> --from X --to Y[:<目標 ID>]`(用 ID 不用名字),table 事件 = 預覽、CLI 自己的確認提示 = 開始;頁面絕不代加 `--yes` / `--force`、不自動回答確認;提示(確認、逐筆裁決、登入精靈、Apple 揭露、授權連結)在精靈裡就地渲染(`Console.run` 的提示容器選項),不切到主控台;Apple 當目的地是灰的並說明、本機曲庫只列既有 M3U;完成報告列出沒搬到的歌;一次一個清單(Q39) | 搬清單的能力(決策 39)與資料來源(`auth status`、`pl list`、migrate 的表)都現成,缺的是一般人看得懂的入口;確認留在伺服器端才守得住「刪改前先看過」的慣例與硬約束;做到一半把人丟進終端機樣貌的主控台等於精靈沒做 |
| 47 | 裝飾動畫與真實進度分開(2026-09-18;計畫 §1) | 首頁的路線示意與三拍說明是裝飾(可循環、reduced-motion 下靜態、命令在跑時停在靜態構圖);真實進度只由新的 `progress` SSE 事件(`stage` / `done` / `total`)驅動,Go 端是預設 no-op 的 package var 接縫,`installWebSeams` 換成送事件,`planResolve` 每首與 migrate / `applyPlans` 的里程碑呼叫;終端機與非 TTY 輸出一個位元組不改;事件還沒有之前不畫進度條 | 參考站的「進度條隨每一首填滿」在我們這裡原本沒有資料來源(#65 的結論:不編百分比);逐首反查是最久的一段,值得給真的數字 |
| 48 | 不用官方標誌、主張只寫事實(2026-09-18;計畫 §1) | 平台用文字 chip + 單色字符,不內嵌 Apple / Spotify 官方 logo;首頁主張限於查得到的事實(免費、開源 MIT、在自己電腦上執行、憑證只進鑰匙圈、正本在自己的 Drive、不刪來源只新增、順序不動、沒有曲數上限),不寫「支援所有平台」「一鍵」、不點名競品;本機 M3U → Spotify 照實主打 | 兩家的品牌規範對 logo 使用都有條件,開源專案內嵌是商標風險;三個平台、其中一個只讀,誇大會在使用者第一次選目的地時就破功 |
| 49 | Apple 寫入的形狀(2026-09-22;計畫 docs/superpowers/plans/2026-09-22-apple-write.md §5–§6) | 建清單 / append 走官方端點(`POST /me/library/playlists`、`POST …/tracks`,`isPublic:false`、每批 100);remove / move 用**一次** `PUT …/tracks` 整批取代(既有列 = 列 id + `library-songs`、新曲 = catalog id + `songs`,重複曲目依出現次數);rename 用 `PATCH`(只送 name);**只寫 `canEdit:true` 的清單**(= 使用者自建;Apple 精選 / 喜好歌曲一律拒絕,寫入前 GET 本體查);`ApplyOps` 自己重讀 `/tracks` 取列 id、對不齊 `current` 就零寫入(這次重讀就是 §6.5.2 規則 6 的併發比對);`CreatePlaylist` 輪詢到列表出現才回傳(上限 30 s);**不用 `DELETE …/tracks`**;不採 rebuild;搬家(T1)不等任何探測、雙向(T2)依 R-8 結果一次 PUT | 全部端點 2026-09-22 真帳號各打過一次(§1.2 表;PUT 對 `i.` 列 204;`a.` 列只在協作清單出現、實測 500 零變動(列數由補測 3 排除到 538 列)→ 協作清單與 canEdit:false 都在 plan 階段跳過(`PlaylistRef.Unwritable`),計畫 §5 補測);`canEdit` 恰好標出全部自建清單,是 §8「只同步自建清單」的機器判準;`mode=all` 會把同一首的兩列一起刪(Cider #1915 重現);不帶 ids 的 DELETE 會清空整份;versionHash 新清單沒有,不能當併發檢查;加進清單的曲目會不會進資料庫看帳號的 Apple Music 設定(2026-09-23 更正:列 id `i.` 不代表在資料庫),揭露要寫但不得寫成必然;協作清單維持唯讀(2026-09-23 定案);風險類別同決策 8(同 host、同 token) |
| 50 | 多語系(2026-09-23;計畫 docs/superpowers/plans/2026-09-23-i18n.md,Q48–Q57 使用者全部採建議) | 預設英文;config `language`(`en` / `zh-TW`,BCP 47)決定,`capy config set language` 與網頁的語言選單改的是同一個設定;不自動偵測作業系統語系、不設環境變數覆寫。`internal/i18n` 是只依賴標準函式庫與 `x/text` 的葉子套件,語系由 cli 層在建命令樹之前設定(`Execute()` 與網頁每個 job);每個語系一個 JSON 目錄(`en.json` 是來源與模板),語意 key、具名佔位符、CLDR 複數;package 層級的錯誤值在印出時才翻(命令內算好的文字欄位不必,語系在命令開始前就定了)。機器欄位不翻譯:TSV 欄名、`action` / `dir` 的值、TSV 最後新增的 `REASON_CODE`(代碼表在 [README](../README.zh-TW.md#reason_code-代碼表),`internal/cli/reason_code_test.go` 比對程式碼與表)、exit code、表頭(四張中文表頭改英文大寫欄名);給人看的文字(`REASON`、stderr、說明、提示)跟著語系。cobra 自己的標題不翻,huh / bubbles 的按鍵說明要翻;訊息裡的可貼命令原樣保留。既有使用者升級後直接變英文,發行說明寫怎麼切回。守門測試:目錄一致、程式用到的 key 都存在、非測試程式碼沒有未搬的中文字串(遷移期間用白名單逐 PR 縮小)。**實作結果(2026-09-23,PR #86–#91;計畫開頭的狀態段)**:目錄 1128 個 key(其中網頁 `webui.*` 277 個),`en` 與 `zh-TW` 兩份。預設在搬字串期間維持 zh-TW,T3(#91)清空白名單時在自己的 commit(`167c528`)改成 `en`(`productionDefault`;測試二進位固定 zh-TW)。網頁:`GET /api/i18n` 是第三個直達端點(§6.6),回 config 的語系、`Supported()`(嵌入的目錄檔)與頁面用的 `webui.*` 加 `webSharedKeys` 列的幾個 CLI key;JS 用 `t(key, {…})` 與 `data-i18n*`,`Intl.PluralRules` 選複數;左欄底部的語言選單走一般命令路徑跑 `config set language`,exit 0 才重新載入;`/api/commands` 在 Serve 前每個語系各算一份。網頁原本五處比對中文輸出的地方全部改掉:帳號頁與搬家精靈讀新的 `capy auth status --json`(各平台 `state` 等事實,欄位只增不改、絕不含 token / secret,schema 在 README)、提示事件帶穩定的 `key`(`confirmWrite(key, text)`)、搬家精靈看 `REASON_CODE`(22 個代碼)、表格找英文欄名、中止時伺服器不再送取消訊息而由頁面看結束原因。守門測試的最終形狀:目錄一致(key、佔位符、單複數形狀、該語言的 CLDR 類別、檔名是正規 BCP 47、**譯文不准是空字串**)、Go 與 JS / HTML 呼叫點的 key 是整個字面、都在 `en.json`、給的佔位符跟目錄一樣、沒人用的 key、非測試 Go 檔與 `internal/cli/webui` 的 JS / HTML / CSS 沒有中日韓字串(註解不算;白名單 `cjkAllowlistLen = 0`,只准縮小)、白名單清空時預設必須是 `en`(`TestDefaultIsEnglishOnceMigrated`)、頁面用到的 key `/api/i18n` 都有送(`TestWebI18nServesEveryKeyThePageUses`)、Apple 揭露在每個嵌入的語系各自完整(`TestAuthLoginAppleDisclosureInEveryLanguage` 跑 `i18n.Supported()` 的每一個,沒寫檢查的語系直接紅)。 | 開源專案預設英文、繁中是維護者自己的語言;一個語系一個檔是新增語系成本最低的模板;`x/text` 已是依賴,不必為複數規則加套件;網頁原本有五處比對中文 CLI 輸出,機器欄位不翻才能讓腳本與網頁都穩定;葉子套件避免 `auth` / `provider` / `canon` 與 `config` / `cli` 繞成循環依賴 |
| 51 | web 播放面板跟著正在播的平台、Spotify 節流(2026-09-24;計畫 docs/superpowers/plans/2026-09-24-web-player-follow-apple-play.md,Q58–Q68 使用者全部採建議;取代決策 42 裡播放面板選平台與輪詢的部分) | **釘住**:`?provider=` 或 `capy --web --provider X` 只問 X,控制鈕也送 X。**自動**:base = 上一輪顯示的平台(還沒有就 config 的 `default_provider`,每輪重讀;`dropNow` 不清它,換語系不會讓面板跳平台);base 在播就顯示它,別家一律不問;沒在播才依序問其他平台(預設先,再照 `providerIDs`),第一個在播的就切過去;都沒在播就留在 base,不管它是暫停、閒置還是出錯——**只有「正在播」能把面板拉走**。**每家一份結果快取與有效期**:Apple 的 State 一律 0,回錯也是(本機 osascript,不花配額);建不起來(沒登入,要讀 keychain)60 s;平台不支援播放(local、Windows 上的 apple)記到 `dropNow` 為止;其他平台的 State 回錯 15 s(token 失效 60 s);429 取 max(60 s, `Retry-After`);Spotify 閒置或暫停 15 s;在播 min(10 s, 本首剩餘 + 1 s)。用快取回答時,在播的進度依經過時間往前推;`play` / `pause` / `next` / `prev` / `seek` / `vol` 經 web 跑完後 3 s 是安定期:命令之前讀到的快取立刻過期(包括出錯的,按了播放就值得再問;限流的冷卻與「不支援播放」除外),安定期內讀到的成功結果不留(問的期間有播放命令跑完的也不留);base 從在播變成沒在播時,其他家的快取立刻過期(同樣不動冷卻與「不支援播放」);`stale_ms` 是距離上一輪多久。只有 `ErrAuthExpired` 丟掉那一家的 controller,Music.app 沒開與限流都是狀態(以前任何錯都全域 `dropNow`,Music.app 關著時每 2.5 s 重建一次 Apple provider)。`dropNow` 只在 `auth login` / `auth logout` / `config set` 之後(唯讀的 `auth status` 不算:首頁每次載入都跑);它保留限流的冷卻,在飛的那一輪結果不寫回快照(世代號);換帳號與 `config set default_provider` 讓面板回到預設平台,換語系不會。輪詢的 ctx 帶 `provider.WithoutWait`:429 不睡在 `pollMu` 裡。Spotify client 會讀 429 的 body:`QUOTA_EXCEEDED` 不重試(沒有 `Retry-After` 就冷卻 10 分鐘),`RateLimitError` 的秒數往上傳——TUI 與 `now --watch` 本來就把限流當狀態,現在第一次認得出 Spotify 的 429,下一次輪詢照 `Retry-After` 等(不照常每 2 s 再打)。Spotify 播 podcast 或廣告(item 是 null)也算正在播。前端:控制鈕與快捷鍵帶面板上那個平台的 `--provider`;每一行前面加平台名稱;曲名連回平台(只收 https;節點原地更新,鍵盤焦點不會每 2.5 s 被丟掉)。Spotify Web API 呼叫量(分頁開著時):Apple 在播 0、都沒在播 4/分、Spotify 在播約 6/分、Spotify 暫停 4/分、分頁藏起來 0(以前顯示 Spotify 時是 24/分,開幾個分頁乘幾倍)。在 web 以外下的命令(終端機、TUI、手機)沒有安定期,面板最多晚一個有效期才反映。研究過、不採用:推播 / webhook(沒有)、Web Playback SDK(只看得到自己那個裝置)、ETag(`/me/player` 沒有,回 304 也算一次請求)、`/me/player/currently-playing`(成本一樣)、Spotify.app 的 AppleScript(在其他 Connect 裝置播放時回報不可靠)、`spotify_cli`(沒有公開文件的 Partner API) | 使用者 2026-09-24 回報:「昨天聽 Apple Music,今天改聽 Spotify,播放列還停在 Apple Music」——面板只問 `default_provider`,控制鈕也不帶 `--provider`。使用者要求依 Spotify 的 Building with AI 指南,找比每分鐘打 24 次 API 更有效的做法:Developer Terms IV 禁止非必要的呼叫;dev mode 的配額從 2026-07 起以開發者帳號計,跟 pl sync、cron 共用;Developer Policy II 要求顯示 Spotify 內容時連回 Spotify。暫停中的歌不能搶面板:Spotify 暫停幾分鐘後會變 204,這時 Music.app 掛著的昨天那首會把面板搶回去,正是這次回報的問題 |
| 52 | Apple 以 id 播放(2026-09-24;計畫 docs/superpowers/plans/2026-09-24-web-player-follow-apple-play.md §2,Q61–Q63 使用者定案,Q63 選項 2) | 先查資料庫:用 `GetTrack` 取 catalog 的歌名、歌手、專輯、時長,以 AppleScript 在本機的 Music 資料庫找歌名與歌手都相同(`whose` 只放文字條件:忽略大小寫、變音符號、NFC/NFD、全半形)、時長差 ≤ 2 s 的曲目;剩好幾首再用專輯篩,剛好一首才用 AppleScript `play` 播,並確認播放中而且目前曲目的 persistent ID 就是它(最多等 5 s),這時才印 ▶。其他情況(對不到、分不開、沒確認到、osascript 出錯如沒有自動化權限)一律 shell `open "music://music.apple.com/<storefront>/song/<id>"`——自己拼單曲網址,取代 P2「歌曲網址一律取 `attributes.url`、不自己拼」;`Play` 回 `*provider.OpenedError`(`errors.Is(ErrOpenedNotPlaying)`,帶「歌名 — 歌手」),CLI 印「已在 Music.app 打開並標出那一首,請對它點兩下」、不印 ▶、exit 0(每次呼叫各自回報,不是能力位)。涵蓋 `play --id`、`play <查詢>`、`artist:`、`--pick`、TUI、web 主控台與搜尋頁;搜尋頁 Apple 列的按鈕叫「在 Music.app 開啟」,命令成功卻不以 ▶ 開頭時把那句顯示在底部。值一律經 osascript 的 argv(`on run argv`,前面加 `--`)傳,不進腳本原始碼。每一步之前先看 ctx:終端機的 Ctrl-C 會連 osascript 一起殺掉,那不是「沒找到」,喊停之後不再開始播放、不再打開 Music.app。刪掉機制 A(`open location`)與 `CAPY_APPLE_PLAY_MECHANISM`。零寫入:不加進資料庫、不動清單,比對結果不落 SQLite / Drive。Windows 照舊不支援播放 | 2026-09-24 macOS 26.5.1 探測(計畫 §2.2,使用者目視):機制 A 連頁面都不換,capy 還謊報 ▶;專輯網址 `?i=` 只到專輯頁;單曲網址跳到專輯頁並標亮那一首,從別的專輯頁出發結果一樣;標亮後下 `play` 播的是原本暫停的那首(所以要叫使用者「點兩下」,不是「按播放」)。2026-09-27 真機:AppleScript 播資料庫裡的訂閱雲端曲目可行,約 0.8 秒確認。比對規則來自使用者資料庫的唯讀實測:同歌名、歌手、時長有三份,只能靠專輯分;catalog 與資料庫的專輯名可能不同(《Overexposed》對《Overexposed (Deluxe Version)》),所以專輯只當第二步;時長一毫秒不差;資料庫的歌手名跟 storefront 一樣是在地化的。不採用:先加進資料庫再播(每按一次就永久寫入、要等 iCloud 同步、202 不代表成功)、UI 自動化(要輔助使用權限、會搶滑鼠、cron 一定失敗)、MusicKit / 瀏覽器播放(要付費會籍、無法指定曲目,或違反 Apple 硬約束) |
