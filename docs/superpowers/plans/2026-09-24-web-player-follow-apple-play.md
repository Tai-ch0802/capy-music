...
狀態:**定案**。使用者 2026-09-24 定案:Q58–Q68 全部照推薦(Q62 授權我跑探測,結果在 §2.2)。PR A(問題一)#95 已合併 → d5dbaf3(決策 51);PR B(問題二,決策 52)開發中。
...
- 按鈕叫「在 Music.app 開啟」(不是「在 Music 開啟」),跟既有字串統一用 Music.app。
- `Play` 回 `*provider.OpenedError`(`errors.Is(ErrOpenedNotPlaying)`),多帶「歌名 — 歌手」:`play --id` 時 CLI 手上只有 id,不然那句會是「已在 Music.app 打開「700050031」」。
- 播放確認只認 persistent ID,不用歌名補判:2026-09-27 使用者授權在他的 Mac 真的播〈Sugar〉,persistent ID 約 0.8 秒就對上;用歌名補判的話,同名的另一首還在播時會誤報 ▶。
- 值經 osascript 的 argv 傳,前面一定加 `--`(開頭是 `-` 的歌名會被當成旗標,`-e` 會被當成原始碼)。
