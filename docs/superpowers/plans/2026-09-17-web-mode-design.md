...
- **disconnected**:SSE 斷線、`/api/now` 連續失敗或連續 5 次 stale 時正在播放列整列換成 `--warn` 左線「capy --web 已停止」+ `r 重新連上`;命令列仍可打字,送出時提示先重連。
