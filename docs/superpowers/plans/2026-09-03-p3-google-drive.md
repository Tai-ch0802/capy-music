...
**2026-09-08:規則已定案並寫進 spec §6.5.1,程式在 `internal/canon/derive.go`。與下面原本的考量有一處不同:對齊鍵是 cid(base 快照同時記觀測當時的 cid,不經 mapping 反查),不是 provider id——同一錄音換版本才會是零變更,重新連結後再刪除也刪得掉;配對由 LCS 決定而不是「第 n 次出現」,重複曲目換序才會一輪就穩。本節其餘為當時的考量,留作紀錄。**
...
**2026-09-08:已實作(分支 `feat/p3-release`)。與上面不同或多出來的地方:** (1) `capy update`(不帶 `--dev`)的正式版路徑一併做了——`update.go` 自己就寫著「等 T10 有 release 再做」:問 `releases/latest`、下載本平台檔與 `checksums.txt`、SHA-256 對得上才解檔、跑一次新 binary 的 `--version` 自檢才覆蓋;任何一步失敗舊的不動。沒有簽章,校驗只保證下載完整(與 release 檔同源)。(2) `release.yml` 要求 `GOOGLE_CLIENT_ID` **與** `GOOGLE_CLIENT_SECRET` 都非空才建:Q1 問的是 Google 要不要 secret、不是有沒有(Desktop client 一定有),空 secret 若 Q1 是「要」會讓所有 release 使用者換 token 時壞掉,寧可讓發版失敗。(3) CI 多一個 `release-dry-run` job:每個 PR 在 macos-latest 跑 `goreleaser release --snapshot`(四個目標交叉編譯、壓縮檔命名、checksums、注入),執行建出來的 darwin/arm64 binary 斷言 `debug google-client` 回 `ci-test`;matrix 也多一步 `go run -ldflags … debug google-client`。`debug google-client` 只印 ID、永遠不印 secret。(4) 目標 darwin / windows × amd64 / arm64(windows/arm64 本機交叉編譯過);Homebrew / Scoop 不做。(5) 檔名契約 `capy_<版本>_<os>_<arch>.tar.gz|zip` 與 `checksums.txt` 在 `.goreleaser.yaml` 與 `update.go` 各寫一份,`TestReleaseAssetNamesMatchGoreleaserConfig` 對照 yaml 原文。(6) 第一個真 release、repo secrets、G-0 都還是維護者的事:release 流程只被 snapshot dry-run 驗過。
