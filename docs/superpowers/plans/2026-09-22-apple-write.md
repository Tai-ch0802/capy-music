...
| 4 | 列 id 前綴 | POST 加入的 6 首全是 **`i.`**(曲庫 id)→ ~~曲目同時進了資料庫~~ **2026-09-23 更正:`i.` 只是資料庫命名空間的 id,不代表那首歌在資料庫;那 7 首現在都不在資料庫(見下方「資料庫副作用的更正」)**;`?representation=resources` 回 200 含新列 id,但**順序與請求不同** | Q46 = **要加揭露**(2026-09-23 改成「看設定、不是必然」);resources 回應不能拿來定位置(反正一次 PUT 不需要) |
...
~~⚠️ 副作用:這次探測把 7 首五月天加進了使用者的 Apple Music 資料庫(刪清單不會連帶移除)。~~ **2026-09-23 更正:那 7 首現在都不在資料庫(catalog `relate=library` 查不到),這次探測沒有留下任何歌;不要照原本那句去刪資料庫裡的歌。**
...
**第三個沒排除的原因:列數**——R-8 的 PUT 只送過 7 列,補測一次送 222 列;500 也可能是 body 大小。無損的分辨法:對非協作、`i.` 列的大清單(「太好聽」543 列)原序全量 PUT 一次——**已做,改用拋棄式複本、不寫原清單,見下方補測 3:列數已排除到 538 列**。
...
判定「Music.app 建的」的依據(PR #82 review 第 3 點):清單本體 `dateAdded` 是 2024-05-06、沒有 `globalId`(沒發佈過)、`hasCatalog:false`——早於本專案,不是任何 API 客戶端建的;25 列裡有 5 首沒有 catalog 對應(library-only),PUT 後也都在。204 之後列 id 逐列不變;清單本體沒有 `versionHash` 欄位;`lastModifiedDate` 變成 PUT 那一刻(2026-09-22T16:57:10Z)。
為什麼用 25 列而不是上面指定的 543 列(PR #82 review 第 2 點):補測 2 驗的是「使用者在 app 裡建、非協作、`i.` 列」這個組合,對列數零推進(7 → 25 列,失敗的那次是 222 列);列數由補測 3 回答。原本寫「543 列仍待 R-9」是掛錯編號(第 1 點):R-9 的成立條件不指定素材,跑過它不代表排除列數。

**補測 3:列數(2026-09-23,使用者授權;「太好聽」只讀不寫)**:讀「太好聽」(543 列、公開、非協作)→ 建一個私人拋棄式清單 → 用它 542 個有 catalog id 的列以 `songs` 分 6 批 POST(100 × 5 + 42,全部 204)→ 複本 538 列 → **一次反序 PUT 538 列**(`library-songs`)→ **204,讀回正好是反序** → 刪複本 204;「太好聽」前後逐列相同。腳本在寫入前確認目標不在建立前的清單列表裡。
...
### 播放列表順序與加入日期(2026-09-23,使用者問)

- **Apple 自己的定義**(Music app 的 AppleScript 說明檔 `/System/Applications/Music.app/Contents/Resources/com.apple.Music.sdef`):`index` =「the index of the item in internal application order」,就是播放列表順序;`date added` =「the date the track was added to the playlist」。**兩個是不同的資料。**
- **誰看得到加入日期**:Mac 的 Music app 可以在清單的歌曲檢視開「加入日期」欄;iPhone 的 Music app 沒有這種排序(Apple 社群 thread 256052152,2025-05 回覆);使用者截圖的排序只有播放列表順序、名稱、類型、年份、藝人、專輯、時間長度,恰好就是 API 每列回傳的欄位;API 沒有任何每列日期(開發者論壇 thread 748222 的功能請求 2024-03 至今無回應;`extend=dateAdded` 也讀不到)。
- **兩者的關係**:只加歌、從不換序時,播放列表順序就是加入的先後(升冪 = 最早加的在前,降冪 = 最新的在前);手動或同步換序後順序變了,加入日期(若有)不變。
- **capy 保證的是播放列表順序**(決策 38 硬約束;補測 2 / 3 的整批取代後逐列比對都對)。加入日期 capy 讀不到也寫不回:只加歌時用 POST 接在尾端,不碰既有列;移除 / 換序用整批取代,Mac 版顯示的加入日期會不會被重設,只有 Mac 的 Music app 看得到。檢查法:跳跳兒歌在 2026-09-22T16:57:10Z 被整批取代過,在 Mac 的 Music app 打開它的歌曲檢視、開「加入日期」欄,25 首若都是那一刻就是會重設(Spotify 會,README 已寫)。README 與指南先寫「還沒驗證」。

### 資料庫副作用的更正(2026-09-23)

R-8 第 4 項以「列 id 是 `i.`」推論「加進清單的曲目同時進了資料庫」,**推論不成立**:`i.` 只是資料庫命名空間的 id。唯讀查(catalog `relate=library`):

| 清單 | 有 catalog 的列 | 其中在資料庫 |
|---|---|---|
| 太好聽 | 542 | 428 |
| 開車車聽的歌單 | 206 | 137 |
| 迪士尼搖籃曲 | 79 | 9 |
...
**使用者 2026-09-22 定案:Q41–Q46 全部採建議**;Q46 依探測第 4 項 = 要加揭露;Q43 探測已跑(§5)。**2026-09-23**:Q46 的揭露改成「看設定、不是必然」(§5 更正);Q47 定案。
...
| Q47 | 協作清單還要不要探測(`a.` 列 id 還是協作本身)、要不要寫 | **不探測、維持唯讀**(使用者 2026-09-23 定案)。就算改用 catalog id 的整批取代可行,capy 也不該這樣改別人也在用的清單:可能把還不在資料庫的歌加進去,「誰加的 / 表情回應」API 讀不到也備份不了 |
