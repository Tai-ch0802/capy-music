...
- **使用者看得到的字串一律走語系目錄(2026-09-23,附錄 C 決策 50;計畫 docs/superpowers/plans/2026-09-23-i18n.md)。** 新字串在 `internal/i18n/locales/` 的 `en.json`(來源)與 `zh-TW.json` 同時補;key 與佔位符名稱寫字串字面(`go test ./internal/i18n/` 靜態檢查);命令名、flag、檔名、平台名不翻;機器欄位(TSV 欄名與 action / dir 的值、exit code、表頭)永遠不翻。搬字串期間預設語系維持 zh-TW(`productionDefault`),`internal/i18n/cjk_allowlist_test.go` 只准縮小,清空的那個 PR 把預設改成 en。Apple 揭露在每個語系都要完整、不可跳過。
