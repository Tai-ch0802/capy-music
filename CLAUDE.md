...
- **隱私權政策要跟程式說的一樣(2026-09-21)。** `site/public/`(https://capy.taislife.work,中英各一份)是 Google OAuth 同意畫面連過去的首頁 / 隱私權政策 / 服務條款。Google scope、存進 Drive 的內容、程式會連的外部服務,任何一項變了,政策必須在**同一個 PR** 更新(`site/site_test.go` 只釘得住 scope);網站維持純靜態——不加 script、分析、cookie、第三方資源,`wrangler.jsonc` 不加 `main` 或 binding。`/guide` 是 `docs/guide.html` 的轉換結果(樣式拆成 `guide.css`、script 拿掉,才過得了網站的 CSP):改了指南要跑 `go test ./site/ -run TestGuideOnSiteIsCurrent -update`、重發 Artifact;repo 接了 Cloudflare Workers Builds(PR 上有建置檢查),合併後要**實際去看線上有沒有更新**,沒有就手動 `cd site && wrangler deploy`。
...
- **使用者看得到的字串一律走語系目錄(2026-09-23,附錄 C 決策 50;計畫 docs/superpowers/plans/2026-09-23-i18n.md)。** 新字串在 `internal/i18n/locales/` 的 `en.json`(來源)與 `zh-TW.json` 同時補;key 與佔位符名稱寫字串字面(`go test ./internal/i18n/` 靜態檢查);命令名、flag、檔名、平台名不翻;機器欄位(TSV 欄名與 action / dir 的值、exit code、表頭)永遠不翻。預設語系是 en(`productionDefault`,測試二進位固定 zh-TW);CJK 白名單已清空,守門測試擋住任何新的中文字面(含 `internal/cli/webui` 的 JS / HTML / CSS,註解不算)與空譯文。Apple 揭露在每個語系都要完整、不可跳過。
