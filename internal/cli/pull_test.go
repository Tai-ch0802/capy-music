...
func TestPlLinkCreate(t *testing.T) {
	...
	if !strings.Contains(errs, "在 spotify 建立清單 公路旅行(new2)") || !strings.Contains(out, "已連結 公路旅行(") || !strings.Contains(out, "spotify:new2") {
	...
	if linked != "new2" {
		t.Fatalf("Drive 上的 公路旅行 要連 spotify:new2,實際 %q", linked)
	}
