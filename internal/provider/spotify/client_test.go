...
func TestSearchArtistsAndTopTracks(t *testing.T) {
	...
	t.Cleanup(srv.Close)
	c := NewClient(srv.Client(), srv.URL)
	artists, err := c.SearchArtists(context.Background(), "五月天", 5)
...
func TestSearchArtistsClampsLimitToPageMax(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("limit") != "10" {
			t.Errorf("limit 應被截到單次上限 10:%s", r.URL.RawQuery)
		}
		w.Write([]byte(`{"artists":{"items":[]}}`))
	}))
	t.Cleanup(srv.Close)
	if _, err := NewClient(srv.Client(), srv.URL).SearchArtists(context.Background(), "x", 50); err != nil {
		t.Fatal(err)
	}
}

func TestPlayContextSendsContextURIOnly(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPut || r.URL.Path != "/me/player/play" || r.URL.Query().Get("device_id") != "d9" {
			t.Errorf("請求形狀錯誤:%s %s?%s", r.Method, r.URL.Path, r.URL.RawQuery)
		}
		var body map[string]any
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			t.Fatal(err)
		}
		if body["context_uri"] != "spotify:playlist:p1" || body["uris"] != nil {
			t.Errorf("body 應只有 context_uri:%v", body)
		}
		w.WriteHeader(http.StatusNoContent)
	}))
	t.Cleanup(srv.Close)
