...
type ISRCParts struct {
	Country     string `json:"country"`     // 前兩碼:國碼,或 Q? / ZZ 這類非地理前綴(見 Geographic)
	Registrant  string `json:"registrant"`  // 三碼登記者代碼
	Year        string `json:"year"`        // 兩碼年份
	Designation string `json:"designation"` // 五碼流水號
	YearFull    int    `json:"year_full"`   // 啟發式(頁面顯示請避免講得像事實):兩碼 ≤ 今年後兩碼 + 1 → 20xx,否則 19xx;+1 是年底為隔年發行預先配號的常態
	Geographic  bool   `json:"geographic"`  // false = 不是國家:nonGeographicPrefixes(QM / QT / QZ / QN 是發行商配到的、ZZ 是國際 ISRC 總部)
}
